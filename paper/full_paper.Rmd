...
header-includes:
 \author{Quang Nguyen\textsuperscript{1}\thanks{Corresponding author. Email\text{:} gmatthews1@luc.edu} , Hannah Butler\textsuperscript{2}, and Gregory J. Matthews\textsuperscript{1}  \vspace{2mm} \\
 ...
 \providecommand{\keywords}[1]{\small \textit{{Keywords}}\text{:} sport climbing, rank-product scoring, sports statistics, social choice theory}
...
For each correlation value, we first generated a population of size $N=100000$ consisting of randomly-drawn values from a standard uniform distribution for the three climbing events speed, bouldering, and lead. Using the algorithm proposed in @Demirtas2019, the lead column was sorted in ascending order, then a fraction of the bouldering column was randomly picked in such a way to induce the desired correlation and these values were ascending-sorted. Once the data were been appropriately sorted, a random sample of either 20 (for qualification) or 8 (for final) rows from the population was selected and then the values were ranked within each discipline. Here, the data generation process ensures that the final data were randomly selected from a population with a specified correlation structure between bouldering and lead. 
...
```{r scores, fig.cap="The average scores for the top 10 qualification ranks and all 8 final ranks, obtained from simulations.  Results shown here are for a Spearman correlation between bouldering and lead of 0.75 with speed assumed independent of both other events.", fig.height=3, fig.width=6.2}
final %>%
  group_by(rank) %>%
