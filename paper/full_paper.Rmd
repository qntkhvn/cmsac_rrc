...
*   **Uniform ranks**. The ranks within each discipline is randomly sampled from a discrete uniform distribution with bounds $[1, 8]$ and $[1, 20]$ for the qualification and final rounds, respectively
...
In order to generate data that satisfy the simulation assumptions, we rely on a framework proposed by @Demirtas2019 for inducing the correlation between bouldering and lead ranks with uniform marginals. 


We first simulated a population that has a desired correlation value by 

generating a marginal of size 100000 then for the two columns of interest, namely, lead and bouldering,

we sorted the lead column in ascending order
then randomly pick out a fraction of the bouldering column associated with the desired correlation value, and sort those randomly-sampled indices

then randomly sample either 8 or 20
then repeat n=10000 times and got the sim results

\ref{fig:corr}


```{r corr, fig.cap="something", fig.height=3.5, fig.width=7}
library(tidyverse)
...
pick spearman correlation of 0.75 as a case study, 
talk about advancement probs and scores, just like previous paper version

put the other stuff in supplementary materials
...
All materials related to this manuscript are publicly available on GitHub at <https://github.com/qntkhvn/cmsac_rrc>.
